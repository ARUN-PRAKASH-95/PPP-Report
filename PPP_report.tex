...
\newpage
\section*{User manual}
\indent\indent\indent\indent The following section deals with the instructions on how to execute the program. In the folder, the main program is present under the name "Refined\_beam.py". The program can be run by typing "python3 Refined\_beam.py" in the terminal. Upon running, the program will ask following inputs from the user

\subsection*{1. Type of cross-section:}
\indent\indent\indent\indent Our implemented model deals with two types of cross-section. 1. Square cross-section  \;2. Elliptical cross-section. The program will ask the user which type of cross-section to be analyzed. For Square cross-section type number (1) in the terminal and elliptical cross-section type number (2) in the terminal. An example of choosing Square cross-section is shown in figure (\ref{fig:tyc}) below 
\begin{figure}[H]
\begin{center}
\includegraphics[width=1\textwidth, height=3cm]{{25.png}}
 \caption{Type of cross-section to be analyzed}
 \label{fig:tyc}
\end{center}
\end{figure}

\subsection*{2. Type of beam element:}
\indent\indent\indent\indent Upon entering the type of cross-section to be analyzed the program will ask for the type of beam element to be used along the beam axis. Three types of beam elements are exploited for the analysis. The input to be given for each type of beam element is shown in the table (\ref{tab:table 11}) below. 
\begin{table}[h!]
  \begin{center}
     \begin{tabular}{c c}
      \hline\\
      \textbf{Element type} & \textbf{Input number} \\
      \\
      \hline
      \\[-2pt]
      ...
      \hline
     \end{tabular}
    \caption{Type of beam element and its respective input number}
    \label{tab:table 11}
  \end{center}
\end{table}
An example of choosing Quadratic element (B3) is shown in figure (\ref{fig:tyb}) below
\begin{figure}[H]
\begin{center}
\includegraphics[width=0.6\textwidth]{{26.png}}
 \caption{Type of beam element employed}
 \label{fig:tyb}
\end{center}
\end{figure}

\subsection*{3. Number of beam elements}
\indent\indent\indent\indent After entering the type of beam element, the program will the ask user for number of elements needed. An example of choosing 40 beam elements is shown in the figure below 
\begin{figure}[H]
\begin{center}
\includegraphics[width=0.6\textwidth]{{27.png}}
 \caption{Number of beam elements utilized}
 \label{fig:tyb}
\end{center}
\end{figure}
